...
### 3. robot_command_handler (NEW!)
웹에서 전송한 명령을 모니터링하고 로봇에 전달합니다.

**발행 토픽:**
- `/robot_command` (std_msgs/String): 로봇 제어 명령 (pause/resume/stop/emergency_stop)

**모니터링 방식:**
- Supabase Realtime으로 `desired_state` 필드 감시
- 5초마다 fallback 폴링

**동작 흐름:**
1. 웹에서 명령 전송 → `desired_state` 업데이트
2. Realtime으로 변경 감지
3. `/robot_command` 토픽으로 명령 발행
4. `doosan_robot_state`와 `status` 업데이트
5. `desired_state` NULL로 초기화 (명령 처리 완료)

**주의:** 이 노드는 Supabase URL/KEY를 하드코딩합니다. 파일을 직접 수정하세요!
