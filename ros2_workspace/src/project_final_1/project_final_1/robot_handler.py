...
def handle_home_request(
    ...
    node = sysctl.node
    node.get_logger().warn("🚑 HOME 명령 수신: 상태 점검 및 복구/이동 시작")

    run_gate.set() # Pause가 걸려있다면 해제

    # 1. 현재 로봇 상태 진단 (독립 노드 활용 권장)
    #    sysctl._standalone_get_state()를 쓰면 좋지만, 
    #    편의상 sysctl.recover_if_possible 내부에서 다시 체크하므로 여기선 DB/Live 값 대조
    live_state = sysctl.get_robot_state(timeout=1.0)
    db_state = get_db_doosan_robot_state(supabase)
    
    # 상태값 결정 (Live 우선)
    current_state = live_state if (live_state is not None and live_state >= 0) else db_state

    node.get_logger().info(f"🧐 현재 상태 진단: {current_state}")

    if current_state is None:
        node.get_logger().error("❌ 상태 확인 불가. 명령 취소.")
        return False

    # 2. 에러 상태(Safe Stop 등)라면 -> 복구(Recovery) 시도
    if current_state in ERROR_STATES:
        node.get_logger().warn(f"🔧 에러 상태({current_state}) 감지! 복구 시퀀스 가동...")
        update_robot_state_row(supabase, status="recovering", recovery_needed=True)

        # [중요] 연쇄 수복(Chain Recovery) 호출
        ok = sysctl.recover_if_possible(int(current_state))
        
        if not ok:
            node.get_logger().error("⛔ 수복 실패. 수동 조치 필요.")
            # 실패했으므로 DB에 여전히 에러라고 표시
            update_robot_state_row(supabase, status="error", recovery_needed=True)
            return False
        
        update_robot_state_row(
            supabase,
            doosan_robot_state = 1, # 1: Standby (가장 유력한 정상 상태)
            recovery_needed=False,
        )
        node.get_logger().info("✅ 수복 완료! 잠시 대기 후 HOME 이동...")
        time.sleep(1.0)

    # 3. 에러가 없거나, 수복에 성공했다면 -> HOME 이동
    #    혹시 Paused 상태라면 Resume
    ...
    node.get_logger().info("🎉 상황 종료: 로봇 정상화 및 HOME 위치 복귀 완료.")
    return True
...
def main(args=None):
    ...
    try:
        # [중요] 방금 완성한 '연쇄 수복' 기능이 있는 Controller를 사용해야 합니다.
    ...
    try:
        while rclpy.ok():
            ...
            if home_event.is_set():
                # 태스크가 실행 중이 아니라면(혹은 에러로 멈췄다면) 홈 명령 처리 시작
                if (not task_busy_event.is_set()) or fault_event.is_set():
                    now = time.time()
                    if now - last_home_try > 0.8:
                        print(f"🏠 Home Command Triggered! (Checking logic...)")
                        last_home_try = now
            ...
            try:
                ...
                # 여기서 루프를 돌면 바로 위의 `if home_event.is_set()`에 걸려서 홈/복구 로직 실행됨

            except StopRequested as e:
                update_task_status(supabase, task_id, "failed")
                print(f"🛑 Stop/Fault requested ({e}) -> task {task_id} aborted")
                # 여기서 루프를 돌면 `fault_event` 때문에 태스크 시작 안 하고 대기함 (DB엔 에러상태 유지)
